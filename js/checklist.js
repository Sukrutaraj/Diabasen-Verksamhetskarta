...
(function () {
  ...
  /* ============================================================
     AUTOMATISK SID-IDENTIFIERING
     Varje tech-sida får egen checklist-mapp
     ============================================================ */
  ...
  async function ghGet(path, token) {
    ...
    if (!r.ok) {
      ...
      /* ============================================================
         Om mappen inte finns ännu → returnera tom lista
         ============================================================ */
      ...
      throw new Error('GitHub GET: ' + r.status);
    }
  ...
  async function ghDelete(path, sha, message, token) {
    ...
    if (!r.ok) {
      throw new Error('Borttagning: ' + r.status);
    }
  }
  ...
  function renderWidget(container) {
      ...
      <div class="cl-box">
        ...
        <details class="cl-add-section">
          ...
          <div class="cl-add-form">
            ...
            <input
              type="url"
              class="cl-input cl-url"
              placeholder="https://sukrutaraj.github.io/digital-checklist-maker/…"
            />

            <label class="cl-label">Ditt namn</label>
            <input
              type="text"
              class="cl-input cl-name"
              placeholder="Förnamn"
            />

            <button class="cl-btn cl-save-btn">
              Spara checklista
            </button>
    ...
    async function loadList() {
      ...
      if (items.length === 0) {
        ...
        listEl.innerHTML =
          '<div class="cl-empty">Inga checklistor tillagda ännu.</div>';
      ...
      items.forEach(item => {
        ...
        div.innerHTML = `
          ...
          >
            ...
            <span class="cl-item-title">
              ${escHtml(item.title)}
            </span>
          </a>

          <span class="cl-item-meta">
            Tillagd av ${escHtml(item.addedBy || '?')}
          </span>

          <button class="cl-del-btn" title="Ta bort">
            🗑
          </button>
        `;

        div.querySelector('.cl-del-btn')
    ...
    savBtn.addEventListener('click', async () => {
      ...
      if (!pwd || !title || !url) {
        ...
        showMsg('Fyll i lösenord, titel och länk.', false);
